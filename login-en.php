...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "SELECT fullname, email, login, password, created_at, 2fa_code, user_type FROM users WHERE login = :login";
      ...
      if (password_verify($_POST['password'], $hashed_password)) {
        ...
        if ($g2fa->verifyCode($row["2fa_code"], $_POST['2fa'], 2)) {
          ...
          if ($row['user_type'] == 'student') {
            header("location: restricted-student-en.php");
          } else if ($row['user_type'] == 'teacher') {
            header("location: restricted-en.php");
          } else {
            echo "<script>alert('User role unknown')</script>";
          }
        } else {
